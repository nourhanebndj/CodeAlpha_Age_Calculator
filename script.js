const ageCalculate = () => {
    ...
    const inputDay = parseInt(document.getElementById("day-input").value);
    const inputMonth = parseInt(document.getElementById("month-input").value);
    const inputYear = parseInt(document.getElementById("year-input").value);

    if (isNaN(inputDay) || isNaN(inputMonth) || isNaN(inputYear)) {
        alert("Please enter valid input for day, month, and year.");
        return;
    }
